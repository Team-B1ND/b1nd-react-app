# react-typescript-rsbuild-axios-boilerplate

CRA를 사용하지 않고 React + TypeScript + Rsbuild + Axios 환경을 위한 보일러플레이트입니다.

이 보일러플레이트는 대표적으로 다음을 지원합니다:

- `JavaScript` 과 `TypeScript` (`jsx`, `tsx`, `js`, `ts`)
...
`tsconfig.app.json`에서 경로 별칭을 설정할 수 있습니다:
...
`src/libs/token/token.ts`에서 토큰을 관리합니다:
...
`src/libs/axios/customAxios.ts`에서 Axios 인스턴스를 사용합니다:
